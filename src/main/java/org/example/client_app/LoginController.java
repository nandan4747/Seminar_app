...
public class LoginController {
    ...
    @FXML private Button admin_login_switch;
    ...
    public void initialize() {
        ...
        admin_login_switch.setStyle("-fx-border-color:linear-gradient(from 0% 0% to 100% 0% ,rgb(255, 27, 27),rgb(109, 0, 252));"+
       ...
       // inner_main_container.setEffect( new GaussianBlur(10));
        // admin button logic
        admin_login_switch.setOnAction(event -> {
            try {
        ...
        double admin_switchX = admin_login_switch.getScaleX();
        double admin_switchY = admin_login_switch.getScaleY();

        admin_login_switch.setOnMouseEntered(mouseEvent -> {
            effect.Hover_effect(admin_login_switch,false,null,null,null,null,1.2,250);
        });
        admin_login_switch.setOnMouseExited(mouseEvent -> {
            effect.Cancel_Effect(admin_login_switch,false,admin_switchX,admin_switchY,null,null,null,null,250);
        });
    }
    ...
    @FXML private void handleLogin(ActionEvent event) {
        ...
        if (!username.isEmpty() && !password.isEmpty()) {
            ...
            connectionManager.push_request("_credential_," + "user_login," + username + "," + password + "," +department);
            do {
            ...
            if (response.equals("<valid>")) {
                ...
                Main_Booking_Application.user_type = "staff";
                try {
                    if (remember) {
                        db_manager.perform_client_login(username, password, department, 1,"staff");
                    } else {
                        db_manager.perform_client_login(username, password, department, 0,"staff");
                    }
